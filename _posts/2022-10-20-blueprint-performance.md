...
If you are a programmer, when go through `UnrealTypes.h`, `CodeGenerator.h` and `Object.h` in source code, you can see the legacy comments in the code that implies reflection types are actually sharing tech debts from old Unreal Engine 3 code.
...
### How Blueprints are interpreted?

Blueprints system is actually very complex and quite difficult to understand unless you have a Ph.D. in scripting languages. 😄
...
But to explain things at most simple level:

- Blueprints graph acts like a *container* of blueprint nodes.
- Every blueprint graph (event graphs, function graphs and macros) are combined into single graph after compilation.
- Each blueprint node is a K2Node (default function nodes are `UK2Node_CallFunction`s that generated by engine) and every K2Node can provide how it's going to be compiled into bytecode. (For example `Sequence` node in blueprints act different between shipping and PIE - it's combined into single linear execution pin to eliminate extra instructions in BP VM)
...
- There is a special method for handling math operations that is suppose to optimize execution speed a little bit more, especially with `Math Expression`s you can get up to 2x more performance. But in the end, it's still going to be bytecode instructions engine will execute.
...
So since it's an embedded language, it can interact with Unreal Engine's existing frameworks, and at most simple level that's what it's all doing. That's why I always tend to think it's actually nothing but a system that keep doing calls to C++ side of the engine. 
...
Blueprints have "direct" and "indirect" overheads. 

**There is only one "direct" overhead of blueprints system:**
...
We can apply same philosophy to Blueprints too. There are many operations and that you can implement in C++ to outperform Blueprints performance by 20x in a regular gameplay code. Because the more less nodes you have in a graph, the less instructions VM will have to execute. **So if you are after optimizing your blueprint code, only thing you can do is reducing the amount of nodes to execute.** 

If you take a look at `Kismet Libraries` that is implemented in Unreal Engine's source code (the static helper functions for math, line traces, overlap checks etc) that's actually what Epic Games trying to do. Handling expensive to run operations in C++ and letting user get away with single function call overhead in the Blueprints graph. Imagine having to loop through all possible overlaps and querying them in blueprints instead of using `Sphere Overlap by Actors`... It would be a terrible experience for your poor CPU! 😛 
